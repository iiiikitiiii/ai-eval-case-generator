...
案例构建mock 当前阶段之前的周期，不要去合理推理后面的

我们应该有一个
journey 与 测试场景（用户角度的需求场景）- 产品功能场景映射表。



我们构建的时候，
上下文要有：根据journey划分的裁剪点病例+journey测试场景+功能场景


统计时候，可以看到当前用例，覆盖了多少场景活journey


minimax:

（API key 已移到 backend/.env 的 MINIMAX_API_KEY，不再明文存在这份笔记里——
这个文件后面很可能被提交进 git，密钥不该留在这种地方。完整 API 参考见
doc/minimaxapi.md，代码里已经按这个文档接好，见 backend/app/services/llm_client.py）
...
（API key 已移到 backend/.env 的 KIMI_API_KEY，不再明文存在这份笔记里，
理由同上面 MiniMax 那条。代码里已经按下面这份文档接好，见
backend/app/services/llm_client.py 的 `_run_kimi`）
...
<CardGroup cols={2}>
  ...
  <Card title="Kimi K3 定价" icon="tag" href="/docs/pricing/chat-k3">
    查看输入与输出价格。
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="模型总览" icon="cubes" href="/docs/api/models-overview">
    了解各模型的能力和参数差异
  </Card>

  <Card title="工具调用" icon="wrench" href="/docs/api/tool-use">
    让模型调用外部函数
  </Card>

  <Card title="创建对话补全" icon="code" href="/docs/api/chat">
    完整的端点参数参考
  </Card>
</CardGroup>
