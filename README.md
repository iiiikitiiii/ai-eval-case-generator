...
| 21（P0，待开发） | **下一轮 query 生成**：复用现有画像、种子用例和模型调用，结合调用端提供的对话历史及上轮真实返回生成下一轮消息；不改动原有 Agent F、静态脚本和导出流程。 |
| 22（P0，待开发） | **单一对外 API**：提供“种子用例 + 画像脚本 + 对话历史 → 下一轮 query／结束标记”接口，复用现有鉴权和图片访问；首版由调用端管理历史，不新增服务端会话表。 |
| 23（P1，待开发） | **最小接入验证**：提供一个 Python 调用示例供现有跑批脚本复用，并从金服测试环境验证同一 API 可用于 Web 或 App 测试；暂不建设专用跑批平台、调试页面和回放看板。 |
